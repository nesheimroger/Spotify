...
import UserProfileView from '../Framework/Components/UserProfile/UserProfileView';
...
function AppMenu(props: Partial<IAppConfig>) {
    ...
    var menus = _.map(props.menus, (x: (IAppMenu)) => {
        ...
        if (isMenuGroup(x)) {
            ...
            return (
                <SubMenu key={x.key} title={x.title}>
                    {items}
                </SubMenu>
            );
        }
    ...
    var matchedGroup: IAppMenuGroup = matchedRoute != null ? _.find(props.menus, (x: IAppMenuGroup | IAppMenuItem) => {
        if (isMenuGroup(x)) {
            return _.find(x.items, (y: IAppMenuItem) => y.key == matchedRoute.key);
        }
    }) : null;

    var selectedKeys = matchedRoute ? [matchedRoute.key] : [];

    var openKeys = matchedGroup != null ? [matchedGroup.key] : [];
...
export default class App extends React.Component<IAppConfig, IAppState> {
    ...
    render() {
        ...
        var routes = _.map(this.props.routes, (x: IAppRoute) => <Route key={x.key} path={x.path} element={x.element}>{x.children}</Route>);
